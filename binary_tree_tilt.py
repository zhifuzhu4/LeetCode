...
and all right subtree node values. If a node does not have a left child, then the sum of the left subtree node values
...
class Solution:
    def findTilt(self, root: Optional[TreeNode]) -> int:
        def helper(node):
            if not node:
                return 0
            lv, rv = helper(node.left), helper(node.right)
            self.res += abs(lv - rv)
