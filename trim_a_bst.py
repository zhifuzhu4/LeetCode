...
class Solution:
    def trimBST(self, root: Optional[TreeNode], low: int, high: int) -> Optional[TreeNode]:
        """
        if root and left subtree is out of lower bound, trim and keep right-subtree
        if root and right subtree is out of upper bound, trim and keep left-subtree
        if root is in range, trim both left and right subtrees
        """
        if not root:
            return None
        elif root.val < low:
